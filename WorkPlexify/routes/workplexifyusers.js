...
const WorkPlexifyUsers = require("../model/workplexifyusers");
...
// Register New user
router.post('/userregister', upload.single('Image'), (req, res, next) => {

    if (!req.file) {
        return res.status(400).json({ error: "No image file provided" });
    }
    // Upload the image to ImageKit
    const folderName = "WorkPlexifyUsers";
    imagekit.upload(
        {
        ...
        (err, result) => {
            ...
            WorkPlexifyUsers.findOne({ email: req.body.email })
                .exec()
                .then(existingUser => {
                    if (existingUser) {
                        ...
                        bcrypt.hash(req.body.password, 10, (err, hash) => {
                            if (err) {
                                ...
                                const useralldata = new WorkPlexifyUsers({
...
router.delete('/deleteuser/:id', (req, res) => {
    WorkPlexifyUsers.deleteOne({ _id: req.params.id })
        .then(result => {
...
router.post('/login', ((req, res, next) => {
    WorkPlexifyUsers.find({ email: req.body.email })
        .exec()
        .then(users => {
            if (users.length < 1) {
...
router.get('/singleuser/:id', ((req, res, next) => {
    WorkPlexifyUsers.findById(req.params.id)
        .then(result => {
            if (result) {
                res.status(200).json({
...
router.put('/updateuserdetail/:id', upload.single('Image'), (req, res, next) => {
    ...
    function updateUserData() {
        ...
        function updateUser() {
            ...
            WorkPlexifyUsers.findByIdAndUpdate(req.params.id, updateUserData, { new: true })
                .then(updatedUser => {
                    if (!updatedUser) {
...
// Update User's Image
router.put('/updateuserimage/:id', upload.single('Image'), (req, res, next) => {
    const file = req.file;
    ...
    imagekit.upload(
        ...
        (err, result) => {
            ...
            WorkPlexifyUsers.findByIdAndUpdate(
                req.params.id,
                { Image: result.url },
                { new: true }
            )
...
router.put('/updateuseractivestatus/:id', (req, res) => {
    ...
    WorkPlexifyUsers.findByIdAndUpdate(req.params.id, updateactiveData, { new: true })
        .then(updatedUser => {
            if (!updatedUser) {
                return res.status(404).json({
                    status: false,
...
router.put('/changepasswordfromaccount', (req, res) => {
    ...
    WorkPlexifyUsers.findOne({ email: email })
        .exec()
        .then(user => {
            if (!user) {
                return res.status(404).json({
...
router.put('/resetpassword', (req, res) => {
    ...
    WorkPlexifyUsers.findOne({ email, phone })
        .exec()
        .then(user => {
            if (!user) {
                return res.status(404).json({
